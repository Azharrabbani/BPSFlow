...
export default function Sidebar({ children, workspace, members, activeWorkspace, activeMembersStatus, getWorkspaceSpaces }) {
    const user = usePage().props.auth.user;

    const { force_reload } = usePage().props;
    ...
    const isAuthorize = ['owner', 'admin'];

    const currentWorkspace = activeWorkspace.workspace.name;
    ...
    const [projectsTrigger, setProjectsTrigger] = useState(false);
    ...
    useEffect(() => {
        const projects = localStorage.getItem('toggleProjects');
        const tasks = localStorage.getItem('toggleTasks');
        setToggleProjects(Number(projects));
        setToggleTasks(Number(tasks));

        if (Number(projects)) {
            getProjects(Number(projects), false);
        }

        if (Number(tasks)) {
            getTasks(Number(tasks), false);
        }
    }, []);

    useEffect(() => {
        window.localStorage.setItem('toggleProjects', toggleProjects);
        window.localStorage.setItem('toggleTasks', toggleTasks);
    }, [toggleProjects, toggleTasks]);
    ...
    // Get Projects
    const getProjects = async (spaceId, triggeredByClick = true, event = null) => {
    if (triggeredByClick && event) {
    ...
    // useEffect(() => {
    //     if (projectsData) {
    //         getProjects( { preventDefault: () => {} }, projectsData);
    //     }
    // }, [projectsTrigger])
    ...
    // Get Tasks
    const getTasks = async (projectId, triggeredByClick = true, event = null) => {
        if (triggeredByClick && event) {
            event.preventDefault();
        }
    ...
    function clearLocalStorage() {
        localStorage.removeItem('toggleProjects');
        localStorage.removeItem('toggleTasks');
    }
            ...
            <div className="h-screen flex bg-blue-100">   
                ...
                {isSidebarOpen && (
                    ...
                    >
                            ...
                            <div className="border-b-2 border-gray-400">
                                ...
                                {currentUserStatus === 'owner' 
                                    ? 
                                        <div>
                                            <Link 
                                                href={route('workspace.edit', activeWorkspace.workspace.id)}
                                            ...
                                            <Link 
                                                href={route('workspace.members', activeWorkspace.workspace.id)}
                                                className="py-3 flex hover:bg-sky-400 hover:text-white hover:rounded-sm transition-colors duration-200 cursor-pointer"
                                            >
                                        ...
                                        {inactiveWorkspaces && inactiveWorkspaces.length > 0 ? (
                                            ...
                                            .map((data) => (
                                                ...
                                                >
                                                    <img src="https://upload.wikimedia.org/wikipedia/commons/2/28/Lambang_Badan_Pusat_Statistik_%28BPS%29_Indonesia.svg" alt="" width="50"/>
                                                    <div className="ml-3">
                                                        <h2 className="text-start">{data.workspace.name}</h2>
                                                    </div>
                                                </button>                                        
                                            ))
                                        ): <p></p>}
                        ...
                        {/* Sidebar Menu Space */}
                        <div className="flex absolute h-[850px] w-full  bg-white flex-col">
                            {/* Sidebar menu */}
                            ...
                            <div>
                                ...
                                <ul className="relative w-full justify-center max-h-[500px] overflow-y-auto">
                                                                    ...
                                                                    projects[space.id].map((project) => (
                                                                        <>
                                                                            <li 
                                                                                ...
                                                                                key={project.id}
                                                                                onMouseOver={() => {
                                                                                    setToggleProjectMore(project.id)
                                                                                }}
                                                                                onMouseLeave={() => {
                                                                            ...
                                                                            >
                                                                                {toggleProjectMore === project.id ? (
                                                                                    <div className="flex items-center gap-2">
                                                                                        <KeyboardArrowDownIcon
                                                                                            className="hover:bg-[#19324928] rounded-md"
                                                                                            onClick={(e) => {
                                                                                                setToggleTasks((toggleTasks) => !toggleTasks);
                                                                                                getTasks(project.id, true, e);
                                                                                            }}
                                                                                        />
                                                                                        <p>{project.name}</p>
                                                                                    </div>
                                        ...
                                        <CreateModal 
                                            open={projectId} 
                                            onClose={() => clear(setProjectId)}
                                        >
                                            <CloseIcon 
                                                className='absolute right-3 top-4 cursor-pointer hover:opacity-50' 
                                                onClick={() => clear(setProjectId)}
                                            />
                                            <div className="mt-1 cursor-default">
                                        ...
                                        <EditModal open={editProjectid} onClose={() => {
                                            setEditProjectId(null);
                                            setData('name', '')
                                        }}>
                                            {editProjectid && (
                                        ...
                                        <CreateModal 
                                            open={taskId} 
                                            onClose={() => clear(setTaskId)}
                                            >
                                            <CloseIcon 
                                                className='absolute right-3 top-4 cursor-pointer hover:opacity-50' 
                                                onClick={() => clear(setTaskId)}
                                            />
                                            <div className="mt-1 cursor-default">
                ...
                <main className="flex-1 md:ml-0 overflow-auto h-full">
                    {children}
                </main>
            </div>
        </>
